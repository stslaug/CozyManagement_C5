...
public class itemPlacement : MonoBehaviour
{
    ...
    public void PrepareToPlaceItem(int flower)
    {

        FlowerType itemType = (FlowerType)flower;
        Debug.Log(itemType.ToString());
        if (gameManager == null) return;

        if (itemType == FlowerType.Fire)
        {
            if (gameManager.saveData.inventoryData.fireFlowerCount > 0)
            {
        ...
        } else if (FlowerType.Water == itemType) {
            if (gameManager.saveData.inventoryData.waterFlowerCount > 0)
            {
        ...
        } else if (FlowerType.Wind == itemType) {
            if (gameManager.saveData.inventoryData.windFlowerCount > 0)
            {
    ...
    private void UpdateInventoryCount()
    {
        ...
        {
            gameManager.saveData.inventoryData.fireFlowerCount--;
        }
        else if (itemToPlace == waterFlowerPrefab)
        {
            gameManager.saveData.inventoryData.waterFlowerCount--;
        }
        else if (itemToPlace == windFlowerPrefab)
        {
            gameManager.saveData.inventoryData.windFlowerCount--;
        }
    }
}
